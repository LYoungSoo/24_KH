...
/* 자료형
    ...
    영어 / 숫자 : 1 BYTE ==> VARCHAR 사용시 4000 글자 작성 가능
    한글 / 한자 : 3 BYTE ==> VARCHAR 사용시 1333 글자 작성 가능
    
    ** CHAR / VARCHAR2 주의사항 **
    - 오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
      UTF-8 은 영어/숫자 1BYTE, 한글 3BYTE로 저장하기 때문에
      한글 데이터 저장 시 크기 계산을 주의해야 한다!!!
    
    NVARCHAR2(문자 개수) : "가변" 길이 문자형 (4000 BYTE)
 ...
 * 이름 				: MEMBER_NAME	/ VARCHAR2(15BYTE) (한글 5자 까지)
...
SELECT MEMBER_ID, MEMBER_PW, MEMBER_NAME, MEMBER_SSN, ENROLL_DATE
FROM "MEMBER"
;
...
-- DESC문 : 테이블의 구조를 표시 (SQLPLUS 에서만 사용 가능) 
-- (DESCRIPTION) (DESENDING 정렬 아님!)
DESC MEMBER;
-- 계정 잘 확인하고
...
/*
    ...
    데이터 무결성 보장을 목적으로 함. --> 중복, NULL, 잘못된 값(주민번호 자리에 숫자 이외 문자) 등
    --> 무결성 보장 == 데이터의 신뢰도 확보
...
--    USER_ID VARCHAR2(20) UNIQUE,	-- 컬럼 레벨 제약 조건(제약 조건명 X)
		USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- 컬럼 레벨 제약 조건(제약 조건명 O)
    								--	CONSTRAINT 제약조건명 "제약조건 종류"
										--> 해당 컬럼에 "USER_ID_U" 라는 이름의 UNIQUE 제약조건을 생성(설정)
--    USER_ID VARCHAR(20),		-- 컬럼레벨 제약조건 미사용
     ...
     * - 제약 조건 종류(컬럼) : 어떤 컬럼에 제약조건을 설정할지 작성 
     */
...
VALUES(1, NULL, 'pass01', '나길동', '남', '010-1234-5678', 'hong123@kh.or.kr')
;
--> ORA-00001: 무결성 제약 조건(KH_LYS.USER_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_LYS.USER_ID_NAME_U)에 위배됩니다
...
CREATE TABLE USER_USED_PK(
    ...
    -- 오류 발생시 이름이 없으면 무슨 키인지 식별이 어렵다고 함
...
;
--> 기본키 중복으로 오류
--  ORA-00001: 무결성 제약 조건(KH_LYS.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr')
;
--> 기본키가 NULL 이므로 오류
--  ORA-01400: NULL을 ("KH_LYS"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_LYS.PK_USERNO_USERID)에 위배됩니다
...
;
-- ORA-01400: NULL을 ("KH_LYS"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
