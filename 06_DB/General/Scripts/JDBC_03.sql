...
AND   SALARY BETWEEN 2000000 AND 7000000
ORDER BY SALARY ASC
;
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE DECODE(MOD(SUBSTR(EMP_NO,8,1),2), 1, 'M', 0, 'F') IN('F')
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE DECODE(MOD(SUBSTR(EMP_NO,8,1),2), 1, 'M', 0, 'F') IN('M','F')
AND   SALARY BETWEEN 4000000 AND 5000000
ORDER BY SALARY ASC
;